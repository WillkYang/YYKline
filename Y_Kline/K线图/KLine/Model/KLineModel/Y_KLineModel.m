...
- (NSString *)V_Date {
    if (!_V_Date) {
        NSDate *date = [NSDate dateWithTimeIntervalSince1970:_Date.doubleValue];
        NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
        formatter.dateFormat = @"yyyy-MM-dd HH:mm";
        NSString *dateStr = [formatter stringFromDate:date];
        _V_Date = [@" " stringByAppendingString: dateStr];
    }
    return _V_Date;
}

- (void)setOpen:(NSNumber *)Open {
    _Open = Open;
    _V_Open =[NSString stringWithFormat:@"%.4f", Open.floatValue];
}

- (void)setClose:(NSNumber *)Close {
    _Close = Close;
    _V_Close = [NSString stringWithFormat:@"%.4f", Close.floatValue];
}

- (void)setHigh:(NSNumber *)High {
    _High = High;
    _V_High = [NSString stringWithFormat:@"%.4f", High.floatValue];
}

- (void)setLow:(NSNumber *)Low {
    _Low = Low;
    _V_Low = [NSString stringWithFormat:@"%.4f", Low.floatValue];
}
