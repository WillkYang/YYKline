...
- (void)event_pichMethod:(UIPinchGestureRecognizer *)pinch {
    ...
    if(ABS(difValue) > Y_StockChartScaleBound) {
        ...
        NSUInteger oldLeftArrCount = ABS((self.pinchCenterX - self.scrollView.contentOffset.x) - [Y_StockChartGlobalVariable kLineGap]) / ([Y_StockChartGlobalVariable kLineGap] + oldKLineWidth);
        NSUInteger newLeftArrCount = ABS((self.pinchCenterX - self.scrollView.contentOffset.x) - [Y_StockChartGlobalVariable kLineGap]) / ([Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth]);
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress
{
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state)
    {
        ...
        [self.verticalView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(@(rightXPosition + self.scrollView.contentOffset.x));
        }];
