...
- (CGFloat)getExactXPositionWithOriginXPosition:(CGFloat)originXPosition{
    __block CGFloat newX = originXPosition;
    [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(Y_KLinePositionModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (originXPosition >= obj.HighPoint.x && originXPosition <= obj.HighPoint.x + [Y_StockChartGlobalVariable kLineWidth]) {
            if(self.delegate && [self.delegate respondsToSelector:@selector(kLineMainViewLongPressKLinePositionModel:kLineModel:)]) {
                [self.delegate kLineMainViewLongPressKLinePositionModel:obj kLineModel:self.needDrawKLineModels[idx]];
            }
            newX = obj.HighPoint.x;
            *stop = YES;
        }
    }];
    return newX;
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
    ...
    for (NSInteger idx = 0 ; idx < kLineModelsCount; ++idx)
    {
        ...
        CGFloat xPosition = idx * ([Y_StockChartGlobalVariable kLineWidth] + [Y_StockChartGlobalVariable kLineGap]) + [Y_StockChartGlobalVariable kLineWidth]/2.f;
